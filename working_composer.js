...
document.addEventListener('keydown', function(event) {
    if(event.keyCode == 37) {
        ...
        if (GWWCurrentTokenElement){
        	...
        	var wordIndex = modelWordIndexFromElementId( id );
...
function tokenForElementId( id ){
	...
	var lineIndex = modelLineIndexFromTokenElementId( id );
	var wordIndex = modelWordIndexFromElementId( id );
	//console.log("TOKENFORLEMENTID lineIndex "+ lineIndex + " wordIndex " + wordIndex);

	//lineIndex and wordIndex point to array in GWWWordTokenModel
...
function modelWordIndexFromElementId( id ){
	var wordIndex;
			...
			keyRadio.onclick = function( event ){
				...
				if (  event.target.getAttribute("disabled") === "true"){
					...
					if (event.target.checked === true ){
						...
						GWWCurrentKeyWordIndex = modelWordIndexFromElementId( GWWCurrentTokenElement.id );
...
function deleteChallenge( element, indexOfChallengeToDelete )
{
	...
	if (challengeBeingDeleted != null){
		...
		var wordIndexOfChallengeBeingDeleted = modelWordIndexFromElementId( element.id );
...
function submitChallenge( event ){
		...
		if (GWWCurrentKeyTokenElement != null){
			...
			var wordIndexOfTokenOfSubmittedChallenge = modelWordIndexFromElementId( GWWCurrentTokenElement.id );
